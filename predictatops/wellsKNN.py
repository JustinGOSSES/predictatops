...
def get_data_for_wellsKNN(input_data_inst):
    ...
    picks_dic = pd.read_csv(
        input_data_inst.picks_dic, input_data_inst.picks_dic_file_path_delimiter
    )
...
def createListOfWellNamesLoadedSplit(wells_df_from_split, config):
    ...
    print(
        "examples of what the well format looks like in wellsLoaded_list",
        wellsLoaded_list[9:12],
    )
...
def findListOfConvertedUWInamesForWellsLoaded(
    wellsLoaded_df_fromh5, UWI, well_format_str
):
    """
    Takes in:
    Returns: 
    ...
    for lasStr in wellsLoaded_list:
        new_wells_loaded_list_inUWIstyle.append(
            changeLASfileToBeUWIstr(lasStr, well_format_str)
        )
...
def makeKNearNeighObj(df_reduced, UWI, Lat, Long, dist, ind, numberNeighbors):
    ...
    UWIs_Geog["Neighbors_Obj"] = pd.Series(
        neighbor_array_per_well_row, index=UWIs.index
    )
    return UWIs_Geog
...
def cleanRenameDF(df, topTarget, thicknessHelperTop, config, input_data_inst):
    ...
    SitID_col, UWI_col, HorID_col, Quality_col, Pick_depth_col = getColNames_for_cleanRenameDF(
        input_data_inst
    )
    lat = input_data_inst.gis_lat_col
    lng = input_data_inst.gis_long_col
    if int(df[HorID_col][0:1][0]) != int(topTarget) or int(
        df[HorID_col + "_x"][0:1][0]
    ) != int(thicknessHelperTop):
        print(df[HorID_col][0:1][0], "should equal topTarget", topTarget)
        print(
            df[HorID_col][0:1][0],
            " should also equal, thicknessHelperTop",
            thicknessHelperTop,
        )
...
def mergeCleanedAndUWIGeog_dfs(df_new_cleaned, UWIs_Geog):
    df_new_cleaned_plus_nn = pd.concat(
        [df_new_cleaned, UWIs_Geog[["Neighbors_Obj"]].copy()], axis=1
    )
...
def broadcastFuncForFindNearestNPickDepth(
    df_new_cleaned_plus_nn, pickColInt, newPickColName, UWI_col
):
    """
    Takes in:
    Returns: 
...
def useThicknessOfNeighborsToEst(df_new2):
    df_new2["NN1_topTarget_DEPTH"] = df_new2["NN1_topTarget_DEPTH"].apply(
        convertStringToFloat
    )
    df_new2["NN1_TopHelper_DEPTH"] = df_new2["NN1_TopHelper_DEPTH"].apply(
        convertStringToFloat
    )
    ...
    df_new2["NN1_thickness"] = (
        df_new2["NN1_TopHelper_DEPTH"] - df_new2["NN1_topTarget_DEPTH"]
    )
    # df_new2['trainOrTest'] = df_new2['trainOrTest']
    df_new2["topTarget_Depth_predBy_NN1thick"] = (
        df_new2["TopHelper_DEPTH"] - df_new2["NN1_thickness"]
    )
    return df_new2
...
def onlyWellsInTestPortion(df, string_train_or_test):
    ...
    print(
        "len(df_just_trainOrMaybeTest['UWI'].unique())",
        len(df_just_trainOrMaybeTest["UWI"].unique()),
    )
...
def fullWellsKNN(wells_df_from_split, input_data_inst, config):
    ...
    print(
        "new_wells_loaded_list_inUWIstyle[5:8]", new_wells_loaded_list_inUWIstyle[5:8]
    )
    print(
        "len(new_wells_loaded_list_inUWIstyle )", len(new_wells_loaded_list_inUWIstyle)
    )
    ################ This next step will reduce the total number of wells to just the ones that have at least the tops and curves we need ################
    ...
    df_new_cleaned = cleanRenameDF(
        df_reduced, target, target_base, config, input_data_inst
    )
...
def getTestRowsOnlyPicksDF(
    df, wellsLoaded_df_fromh5_newUWI, config, whichTrainOrTest_str, well_file_ending
):
    #### funtion that takes in curves df and gets lists of test only wells
    wells_test_only = wellsLoaded_df_fromh5_newUWI.loc[
        wellsLoaded_df_fromh5_newUWI[config.trainOrTest] == whichTrainOrTest_str
    ]
