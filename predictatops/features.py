...
def get_wellsKNN_results(output_data_inst):
    ...
    full_path_to_wellsKNN_results = getMainDFsavedInStep(
...
def get_split_curve_results(output_data_inst):
    ...
    full_path_to_split_curve_results = getMainDFsavedInStep(
        path_to_prev_results, path_to_directory, file_name, ending
    )
    ##### load dataframe from full path
    wells_df_from_split_curveData = load_prev_results_at_path(
        full_path_to_split_curve_results
    )
...
def mergeCurvesAndTopsDF(wells_df_from_split_curveData, wells_df_from_wellsKNN, config):
    ...
    df_all_wells_wKNN = pd.merge(
        wells_df_from_split_curveData, wells_df_from_wellsKNN, on=config.UWI
    )
...
def convertAllColButGivenToFloat(config, df_all_wells_wKNN):
    ...
    print(
        "turning all columns but these into floats. Should come from config.colsToNotTurnToFloats",
        keepStringsArray,
    )
    columns = list(df_all_wells_wKNN.columns.values)
    columns_to_turn_to_floats = [
        item for item in columns if item not in keepStringsArray
    ]
    df_all_wells_wKNN[columns_to_turn_to_floats] = df_all_wells_wKNN[
        columns_to_turn_to_floats
    ].astype(float)
    return df_all_wells_wKNN
...
def createDepthRelToKnownTopInSameWell(df):
    ...
    #### for top McMurray
    df_all_wells_wKNN_DEPTHtoDEPT["diff_TopTarget_DEPTH_v_rowDEPT"] = (
        df_all_wells_wKNN_DEPTHtoDEPT["TopTarget_DEPTH"]
        - df_all_wells_wKNN_DEPTHtoDEPT["DEPT"]
    )
    #### for base McMurray or Top Paleozoic
    df_all_wells_wKNN_DEPTHtoDEPT["diff_TopHelper_DEPTH_v_rowDEPT"] = (
        df_all_wells_wKNN_DEPTHtoDEPT["TopHelper_DEPTH"]
        - df_all_wells_wKNN_DEPTHtoDEPT["DEPT"]
    )
...
def createFeat_withinZoneOfKnownPick(df, config):
    ...
    ] = df_all_wells_wKNN_DEPTHtoDEPT["diff_TopTarget_DEPTH_v_rowDEPT"].apply(
        lambda x: zones[0]
        if x == zonesAroundTops[zones[0]][0]
        else (
            zones[1]
            if (zonesAroundTops[zones[1]][0] < x and x <= zonesAroundTops[zones[1]][1])
            else zones[2]
            if (zonesAroundTops[zones[2]][0] < x and x <= zonesAroundTops[zones[2]][1])
            else zones[3]
            if (zonesAroundTops[zones[3]][0] < x and x <= zonesAroundTops[zones[3]][1])
            else zones[4]
        )
    )
...
def NN1_TopMcMDepth_Abs(df, config):
    ...
    df["DistFrom_NN1ThickPredTopDepth_toRowDept"] = abs(
        df[col_topTarget_Depth_predBy_NN1thick] - df[DEPT]
    )
    return df
...
def markingEdgeOfWells(df, config):
    ...
    df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["NewWell"] = (
        df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["UWI"].shift(1)
        != df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["UWI"]
    )
    df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["LastBitWell"] = (
        df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["UWI"].shift(-1)
        != df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["UWI"]
    )

    TopOfWellRowsOnly = df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM.loc[
        df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["NewWell"] == True
    ]
    BottomOfWellRowsOnly = df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM.loc[
        df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["LastBitWell"] == True
    ]
    ...
    TopAndBottomOfWellRowsOnly = pd.merge(
        TopOfWellRowsOnly, BottomOfWellRowsOnly, on="UWI"
    )
    #### merge with larger dataframe
    df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM = pd.merge(
        df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM,
        TopAndBottomOfWellRowsOnly,
        on="UWI",
    )

    ####
    #### Create a col for distance from row to top of well
    df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["FromTopWell"] = (
        df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["DEPT"]
        - df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["TopWellDept"]
    )

    #### Create a col for distance from row to bottom of well
    df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["FromBotWell"] = (
        df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["BotWellDept"]
        - df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["DEPT"]
    )

    #### Create col for well total thickness measured
    df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["WellThickness"] = (
        df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["BotWellDept"]
        - df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["TopWellDept"]
    )
    ...
    #### This adds a column that says whether a row is closer to the bottm or the top of the well
    #### This is useful for doing creation of features of rolling windows where you want to avoid going into another well stacked above.
    df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["closerToBotOrTop"] = np.where(
        df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["FromTopWell"]
        <= df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["FromBotWell"],
        "FromTopWell",
        "FromBotWell",
    )
    df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["closTopBotDist"] = np.where(
        df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["FromTopWell"]
        <= df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["FromBotWell"],
        df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["FromTopWell"],
        df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["FromBotWell"],
    )
    df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["rowsToEdge"] = (
        df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["closTopBotDist"] / 0.25
    )
    df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM[
        "rowsToEdge"
    ] = df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["rowsToEdge"].astype(int)

    df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["diff_DEPT_vs_NN1_topTarget_DEPTH"] = (
        df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["DEPT"]
        - df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM["NN1_topTarget_DEPTH"]
    )
...
def thoughts_seperateRollingAndConditionalIntoTwoDaskProcesses(dd, curves, windows):
    ...
    for eachArgList in all_comboArgs_B:
        ...
        #### MIN
        dd[featureName + "dir" + "Around" + "Min"] = (
            dd[col].rolling(windowSize, center=True).min()
        )
        dd[featureName + "dir" + "Around" + "Min"] = dd[
            featureName + "dir" + "Around" + "Min"
        ].where(cond=dd["closTopBotDist"] > half_window, other=dd[col])

        dd[featureName + "dir" + "Above" + "Min"] = (
            dd[col].rolling(windowSize, center=False).min()
        )
        dd[featureName + "dir" + "Above" + "Min"] = dd[
            featureName + "dir" + "Above" + "Min"
        ].where(cond=dd["closTopBotDist"] > windowSize, other=dd[col])
        print(
            "finished min in function thoughts_seperateRollingAndConditionalIntoTwoDaskProcesses"
        )
        ### MAX
        dd[featureName + "dir" + "Around" + "Max"] = (
            dd[col].rolling(windowSize, center=True).max()
        )
        dd[featureName + "dir" + "Around" + "Max"] = dd[
            featureName + "dir" + "Around" + "Max"
        ].where(cond=dd["closTopBotDist"] > half_window, other=dd[col])

        dd[featureName + "dir" + "Above" + "Max"] = (
            dd[col].rolling(windowSize, center=False).max()
        )
        dd[featureName + "dir" + "Above" + "Max"] = dd[
            featureName + "dir" + "Above" + "Max"
        ].where(cond=dd["closTopBotDist"] > windowSize, other=dd[col])
        #### Mean
        dd[featureName + "dir" + "Around" + "Mean"] = (
            dd[col].rolling(windowSize, center=True).mean()
        )
        dd[featureName + "dir" + "Around" + "Mean"] = dd[
            featureName + "dir" + "Around" + "Mean"
        ].where(cond=dd["closTopBotDist"] > half_window, other=dd[col])

        dd[featureName + "dir" + "Above" + "Mean"] = (
            dd[col].rolling(windowSize, center=False).mean()
        )
        dd[featureName + "dir" + "Above" + "Mean"] = dd[
            featureName + "dir" + "Above" + "Mean"
        ].where(cond=dd["closTopBotDist"] > windowSize, other=dd[col])
        print(
            "finished mean in function thoughts_seperateRollingAndConditionalIntoTwoDaskProcesses"
        )
        ## nLargest
        nValues = 5
        dd[featureName + "dir" + "Above" + "nLarge"] = (
            dd[col]
            .rolling(windowSize, center=False)
            .apply(lambda x: nLargest(x, nValues), raw=True)
        )
        dd[featureName + "dir" + "Above" + "nLarge"] = dd[
            featureName + "dir" + "Above" + "nLarge"
        ].where(cond=dd["closTopBotDist"] > windowSize, other=dd[col])

        dd[featureName + "dir" + "Around" + "nLarge"] = (
            dd[col]
            .rolling(windowSize, center=True)
            .apply(lambda x: nLargest(x, nValues), raw=True)
        )
        dd[featureName + "dir" + "Around" + "nLarge"] = dd[
            featureName + "dir" + "Around" + "nLarge"
        ].where(cond=dd["closTopBotDist"] > windowSize, other=dd[col])
...
def createManyFeatFromCurvesOverWindows_withDask(df, config):
    ...
    ddf_test5 = thoughts_seperateRollingAndConditionalIntoTwoDaskProcesses(
        test_5, curves, windows
    )
    print(
        "NOTE: currently in createManyFeatFromCurvesOverWindows(df,config) function & calculated graph for ddf_test5. This will take a while to compute() via Dask, so sit tight!"
    )
...
def createManyFeatFromCurvesOverWindows_withOutDask(df, config):
    ...
    #### of total compute time currently!
    print(
        "started to run long function to create features in createManyFeatFromCurvesOverWindows_withOutDask(df,config), sit tight!"
    )
    df_test5 = thoughts_seperateRollingAndConditionalIntoTwoDaskProcesses(
        df_all_wells_wKNN_DEPTHtoDEPT_KNN1PredTopMcM_NearTop, curves, windows
    )
