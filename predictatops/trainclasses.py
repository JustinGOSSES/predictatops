...
class ML_obj_class:
    ...
    def __init__(self, output_data_inst):
        ...
        self.machine_learning_dir = (
            output_data_inst.base_path_for_all_results
            + "/"
            + output_data_inst.path_balance
        )
        self.h5_to_load = (
    ...
    def check_test_df_same_size(self):
        ...
        if len(self.test_X) == len(self.test_y) and len(self.test_y) == len(
            self.test_index
        ):
            response = (
                "PASSED: test_X and test_y and test_index are all the same size as asserted "
                + str(len(self.test_X))
            )
        else:
            response = (
                "FAIL?: test_X and test_y in the test dataset should be the same size BUT THEY ARE NOT!!! test_X = "
                + str(len(self.test_X))
                + " and test_y ="
                + str(len(self.test_y))
                + " and test_index="
                + str(len(self.test_index))
            )
        return response

    def check_train_df_same_size(self):
        """doc string goes here"""
        if len(self.train_X) == len(self.train_y) and len(self.train_y) == len(
            self.train_index
        ):
            response = (
                "PASSED: train_X and train_y and train_index are all the same size as asserted "
                + str(len(self.train_X))
            )
        else:
            response = (
                "FAIL?: train_X and train_y and train_index should be the same size BUT THEY ARE NOT!!! train_X = "
                + str(len(self.train_X))
                + " and train_y ="
                + str(len(self.train_y))
                + " and train_index="
                + str(len(self.train_index))
            )
        return response
    ...
    def dropNeighbors_ObjCol(self, col_list):
        ...
        try:
            ...
            print(
                "dropped :",
                col_list,
                " in train_X, test_X, and preSplitpreBal dataframes",
            )
        except:
            print(
                "Could not find something in ",
                col_list,
                " and such did not drop :",
                col_list,
                " in train_X, test_X, and preSplitpreBal dataframes. It may nto exist.",
            )
    ...
    def load_data_for_ml(self):
        ...
        doc string goes here
        """
        self.train_X = pd.read_hdf(
            self.machine_learning_dir + "/" + self.h5_to_load, "train_X"
        )
        self.train_y = pd.read_hdf(
            self.machine_learning_dir + "/" + self.h5_to_load, "train_y"
        )
        self.test_X = pd.read_hdf(
            self.machine_learning_dir + "/" + self.h5_to_load, "test_X"
        )
        self.test_y = pd.read_hdf(
            self.machine_learning_dir + "/" + self.h5_to_load, "test_y"
        )
        self.train_index = pd.read_hdf(
            self.machine_learning_dir + "/" + self.h5_to_load, "train_index"
        )
        self.test_index = pd.read_hdf(
            self.machine_learning_dir + "/" + self.h5_to_load, "test_index"
        )
        self.preSplitpreBal = pd.read_hdf(
            self.machine_learning_dir + "/" + self.h5_to_load, "preSplitpreBal"
        )
...
def saveTrainClassesResultsAsPickle(model, MLinstance, output_data_inst):
    ...
    load_results_full_file_path = (
        load_dir
        + "/"
        + output_data_inst.trainclasses_results_model
        + output_data_inst.default_results_file_format
    )
    ...
    print(
        "finished saving the results of the model step in the location set in the output class instance. = ",
        joblib_file,
    )
