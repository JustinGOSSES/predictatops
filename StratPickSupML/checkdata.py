...
class TopsAvailable():
    ...
    def __init__(self,input_data_obj,configuration_obj):
        ...
        self.wells_with_all_given_tops = None # populated in function below within this class
 ...
 #### THIS ONE NEEDS EDITING AS IT REQUIRES ARGUMENTS THAT MAY NOT BE NEEDED LIKE THIS????????   
    ...
    def run_all(self):
        ...
        print("The length of the list with all the names of the wells that have the required tops"+str(self.get_must_have_tops())+"is: ",len(self.wells_with_all_given_tops), " and it will be what is returned")
...
class CurvesAvailable():
    ...
    def __init__(self,input_data_obj,configuration_obj):
        ...
        self.onlyPlentifulCurvesArray = None # populates from getCurvesInMinNumberOfWells
        self.wellsWithWantedCurves = None # populates from findWellsWithCertainCurves(self)
        #self.picks_df_noNullPicks = "nothing here yet, run take_out_wells_with_no_tops or set_picks_df_noNullPicks"
        #self.wells_wAny_tops__list = "nothing here yet"
    ...
    def getCurvesInMinNumberOfWells(self):
        """ say what it does here"""
        minNumberCurves = self.config.threshold_returnCurvesThatArePresentInThisManyWells
    ...
    def findWellsWithCertainCurves(self):
        ...
        if self.onlyPlentifulCurvesArray == None:
            print("self.onlyPlentifulCurvesArray was not populated yet so getCurvesInMinNumberOfWells() will be ran")
        ...
        if self.countsOfCurvesObj == None:
            print("self.countsOfCurvesObj was not populated yet so countsOfCurves()) will be ran")
            self.countsOfCurves()
    ...
    def run_all(self):
        ...
        all_results_obj = {"countsOfCurves":countsOfCurves,"objectOfCurves":objectOfCurves,"onlyPlentifulCurvesArray":onlyPlentifulCurvesArray,"wellsWithWantedCurves":wellsWithWantedCurves}
        print("returning an object with 4 parts, each a piece of information that helps determine how many log curves are availabe in how many logs ")
...
def getCurvesListWithDifferentCurveName(originalCurveList,origCurve,newCurve):
    ...
    try:
        ...
        print("did not find ",origCurve, " when attempting to replace it with ",newCurve," in the getCurvesListWithDifferentCurveName function")
